...
#End Region
    ...
    Private Sub btn_addserv_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_addserv.Click
        ...
        If service_type.Count > 0 And cus_id <> "" And servdatefindprom <> "" Then
            ...
            For i As Integer = 0 To service_type.Count - 1
                If service_empid(i) <> "" Then
                    If service_type(i) = "S" Then
                        normalservice.Add(service_id(i) & "|" & service_empid(i))
                    ElseIf service_type(i) = "P" Then
                        promotionservice.Add(service_id(i) & "|" & service_empid(i))
                    End If
                Else
                    MsgBox("กรุณาเลือกพนักงานที่จะบริการให้ครบ")
                    Exit Sub
                End If
                ...
                For Each rowserv As DataGridViewRow In grid_showservlist.Rows
                    ...
                    If normalservice.Count > 0 Then
                        For Each normalserv As String In normalservice
                            Dim normalservsplit As String() = normalserv.Split("|")
                            If rowserv.Cells(5).Value = normalservsplit(0) And normalservsplit(1) <> "" Then
                                sql &= " insert into service_detail (serv_id,emp_id,liserv_id) values " &
                                    " ( " & servid & "," & normalservsplit(1) & "," & rowserv.Cells(5).Value & " )"
                                normaladd = True
                                Exit For
                            End If
                        Next
                    End If
                    If normaladd = False Then
                        sql &= " insert into service_detail (serv_id,liserv_id) values " &
                               " ( " & servid & "," & rowserv.Cells(5).Value & " )"
                    End If
                Next
                If Obj_query.DMLData(sql) Then
                    sql = "insert into bill (serv_id,bill_date) values (" & servid & ", '" & servdatefindprom & "' )"
                    ...
                    If Obj_query.DMLData(sql) Then
                        ...
                        If promotionservice.Count > 0 Then
                            For Each promserv As String In promotionservice
                                Dim promservsplit As String() = promserv.Split("|")
                                sql &= "insert into bill_detail (prom_id,bill_id,emp_id) values " &
                                     " ( " & promservsplit(0) & "," & billid & "," & promservsplit(1) & " )"
                            Next
                            If Obj_query.DMLData(sql) Then
    ...
    Private Sub grid_servresult_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grid_servresult.CellClick
        ...
        " , convert(varchar,liserv_price,1) 'ราคา' , c.emp_fullname , d.liserv_id , c.emp_id " &
        " from service a(nolock) " &
        " join service_detail b(nolock) on a.serv_id =b.serv_id " &
        " left join employee c(nolock) on b.emp_id = c.emp_id " &
        ...
        For i As Integer = 0 To detailservicelist.Rows.Count - 1
            ...
            Dim servprice As String = detailservicelist(i)(3).ToString()
            Dim servid As String = detailservicelist(i)(5).ToString()
            Dim empid As String = detailservicelist(i)(6).ToString()
            Dim empname As String = detailservicelist(i)(4).ToString()
        ...
        If nowadd = "prom" Then
            ...
            For Each promlistforchk As DataGridViewRow In grid_chooseprom.Rows
                ...
                If thispromlist_count = 0 Then
                    grid_chooseprom.CurrentCell = grid_chooseprom.Item(0, rownow)
                    promlistforchk.Cells(0).Value = True
                    grid_chooseprom.UpdateCellValue(0, rownow)

                    Dim promservnew As DataTable = get_Promlistall(grid_chooseprom.Rows(rownow).Cells(6).Value) ' โปรโมชั่นที่เลือก
                    ...
                    Next
                    If grid_chooseprom.Rows(rownow).Cells(2).Value = "ส่วนลด" Then
                    ...
                    ElseIf grid_chooseprom.Rows(rownow).Cells(2).Value = "แถม-ฟรี" Then
                        ...
                        pricenow += pricedo
                    ElseIf grid_chooseprom.Rows(rownow).Cells(2).Value = "แพ็คเกจ" Then
                        ...
                        If grid_chooseprom.Rows(rownow).Cells(5).Value = "F" Then
    ...
    Private Sub grid_packet_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grid_packet.CellContentClick
        ...
        If grid_packet.CurrentCell.ColumnIndex.Equals(0) Then
            ...
            For Each rowprom As DataGridViewRow In grid_chooseprom.Rows
                If rowprom.Cells(0).Value = True Then
                    Dim flagdup As Boolean = False
                    Dim promname As String = ""
                    Dim prompacket As DataTable = get_Promlistall(rowprom.Cells(6).Value)
